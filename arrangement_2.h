// Declaration of Window class : functions and attributes
...
private:
    ...
    /*! read from file */
    void load(bool indicator, const QString& filename , bool clear_flag = false);
...
private slots:
   ...
   // void add_segment_tab();
    ...
    void zoomout();
    /*! open a file */
    void fileOpen(bool indicator, bool clear_flag = false);
   // prescanner
    QString prescanInput( const QString& filename );
    /*! open a Pm file */
    void fileOpenPm();
 ...
 //   void setConicType( ConicType t );
	...
	//cartogram relevant functions and slots
	void cartogram_balance();
	void cartogram_weights();
	void cartogram_it();
	void cartogram_start();
	float FaceArea(Conic_face_handle f);
	void setUpWeights();
	My_polygon extractRoughPolygonFromFace(Conic_face_handle f);
	My_polygon extractFinePolygonFromFace(Conic_face_handle f, float precision);
	My_polygon_K extractPolygonKFromFace(Conic_face_handle f);
	dualGraph extractDualFromArrangement(primalMap *indPrimalFaces, dualMap *indDualVertices);
	void print_all_weights();
	double calcBendingCapacityEstimate(Conic_face_handle fhan,Conic_halfedge_handle hhan);
	double calcBendingCapacityExact(Conic_face_handle fhan,Conic_halfedge_handle hhan,primalMap *indPrimalFaces);
	bool compareEdges(Conic_halfedge_handle h1,Halfedge_handle h2);
	double calculateAngle(Halfedge_handle hh1, Halfedge_handle hh2, K_point p1);
	double calculateAngle(Conic_halfedge_handle hh1, Conic_halfedge_handle hh2, Arr_conic_point_2 p1);
	double areaToArcHeight(double a, Arr_conic_point_2 p1, Arr_conic_point_2 p2);
	void bendEdge(Conic_halfedge_handle hhan, double h);
	Arr_conic_2 obtainCircularArc(Conic_halfedge_handle hhan, double h);
	void showSkeleton(Conic_face_handle fhan );
	void showSkeletonFace(Conic_face_handle fhan,Conic_halfedge_handle hhan);
	void applyFlow(dualGraph *dual, primalMap *indPrimalFaces, dualMap *indDualVertices);
	void printDualToFile(dualGraph dual);
	void store(dualGraph dual,primalMap *indPrimalFaces, dualMap *indDualVertices,QString filename);
	bool vertexIsReflex(Conic_halfedge_handle hhan, Arr_conic_point_2 p);
	My_polygon_K getBoundingPolygon();
	My_polygon_K getContinentPolygon(int conti);
	void clusterContinents();
	int searchConti(Conic_face_handle fhan);
	void printSkeleton(SsPtr skeleton);
	void storeStraight(QString filename);
...
/*! old state of current tab */
    ...
    /*! array of colors */
    QColor *colors;
    // holds the original arrangement with no bendings
    map<int,SsPtr> skeletons;
    //holds all edges of the subdivision bordering the outer face
    vector<primal_edge> outerFaceEdges;
    //stocks the number of continents (islands in the subdivision)
    int numberOfContinents;
    bool debug ;

	// Cartogram actions
	QAction *carto_new;
	QAction *carto_weights;
	QAction *carto_it;
	QAction *carto_start;
